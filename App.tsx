...
const App: React.FC = () => {
  ...
  return (
    <div className="flex flex-col h-full max-w-lg mx-auto bg-white dark:bg-slate-950 shadow-[0_60px_130px_-40px_rgba(0,0,0,0.18)] relative print:max-w-none print:shadow-none font-sans overflow-hidden">
      ...
      {/* Bottom Navigation */}
      <div className="bg-white dark:bg-slate-950 border-t border-slate-50 dark:border-slate-900 px-10 py-4 flex justify-between items-center z-20 pb-safe no-print shadow-[0_-20px_50px_-15px_rgba(0,0,0,0.04)]">
        <NavButton 
...
const NavButton: React.FC<{ active: boolean; onClick: () => void; icon: React.ReactNode; label: string }> = ({ active, onClick, icon, label }) => (
    <button 
        onClick={onClick}
        className={`flex flex-col items-center justify-center w-20 py-1 transition-all duration-300 active:scale-90 ${active ? 'text-indigo-600 dark:text-indigo-400' : 'text-slate-300 dark:text-slate-700 hover:text-slate-500'}`}
    >
        <div className={`p-2.5 rounded-[22px] transition-all duration-300 ${active ? 'bg-indigo-50 dark:bg-indigo-950 ring-8 ring-indigo-50/40 dark:ring-indigo-900/20 translate-y-[-6px] shadow-sm' : ''}`}>
            {icon}
        </div>
